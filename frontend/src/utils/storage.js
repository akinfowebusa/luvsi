...
export const setItem = async (key, value) => {
  try {
    const jsonValue =
      typeof value === 'string' ? value : JSON.stringify(value); // ✅ string ko direct store karo
...
export const getItem = async (key) => {
  try {
    ...
    // ✅ Try to parse if JSON hai
    try {
      return JSON.parse(value);
    } catch {
      return value; // agar normal string hai to direct return
    }
  } catch (error) {
